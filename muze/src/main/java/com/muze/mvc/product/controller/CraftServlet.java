...
import com.muze.mvc.product.model.service.CraftService;


@WebServlet(name = "craftServlet", urlPatterns = { "/product/craft" })
public class CraftServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	listCount = new CraftService().getPaintingCount(); // 페이지 숫자를 넘버링하기 위한 총 리스트 갯수
    	pageInfo = new PageInfo(page, 10, listCount, 4); // 페이지 정보 만들기
    	list = new CraftService().getPaingingList(pageInfo); // 페이지 별 실제 리스트 개수
